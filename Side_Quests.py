     ...
     if main_hero.class_character == "Warrior":
        if main_hero.side_quest == "Meet Nathan on the Site" and main_hero.tournament_site == None:
                    ...
                    if not location__current_fight(main_hero, enemy):
                        ...
                        main_hero.side_quest = None
                        slow_print("Judge: And the last year Champion goes to his second award\n")
                        time.sleep(2)
                        slow_print("Champion: You did well, man. Finaaly I was fighting seriosly for a long time.\nHope to see in the next year\n")
                        time.sleep(2)
                        slow_print("You received 20 gold and 125 Exp\n")
                        main_hero.gold += 20
                        main_hero.exp += 125
                        return
                    main_hero.update_stats()
                    ...
                    if not location__current_fight(main_hero, enemy):
                        ...
                        main_hero.side_quest = None
                        slow_print("Judge: Nathan became the new Champion of the Tournament!!!\n")
                        time.sleep(2)
                        slow_print("Nathan:That was so close, mate, you almost did it. I hope we meet again soon, but now I need to go with my commander. See ya\n")
                        time.sleep(2)
                        slow_print("You received 30 gold and 150 Exp\n")
                        main_hero.gold += 30
                        main_hero.exp += 150
                        main_hero.update_stats()
                    main_hero.update_stats()
